...
# set parameters
n_epoch = 50000 # 50000 # 1000000 # number of epochs
...
def evaluate(data, targets, test=False):
    ...
    for i in six.moves.range(len(targets)):
        ...
        x_batch = np.array([one_data + one_hot_target],dtype ='float32')
    ...
    while epoch <= n_epoch:
        ...
        for i in six.moves.range(jump):
            if   train_data[i] == [1, 0.5]:
                one_data = [1, 0, 0, 0]
            elif train_data [i] == [0.5, 1]:
                one_data = [0, 1, 0, 0]
            elif train_data [i] == [0.5, 0]:
                one_data = [1, 0, 0, 0]
            elif train_data [i] == [0, 0.5]:
                one_data = [1, 0, 0, 0]
            
            one_hot_target = inilist = [0] * 81
            if  train_targets[i]%3==0:
                #if  i%2 == 0:
