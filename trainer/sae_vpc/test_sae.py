...
for n in range(0, N):
    ...
    print(err.data)
    plt.subplot(3, 1, 1)
    plt.imshow(np.flipud(x.data.reshape((90, 12)).T), cmap=plt.cm.gray, interpolation='none')
    plt.subplot(3, 1, 2)
    plt.imshow(np.flipud(y.data.reshape((90, 12)).T), cmap=plt.cm.gray, interpolation='none')
    plt.subplot(3, 1, 3)
    plt.imshow(np.absolute(np.flipud((x.data - y.data).reshape((90, 12)).T)), cmap=plt.cm.gray, interpolation='none')
    plt.show()
